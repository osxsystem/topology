...
# Build a fake root with a VERSION file that would resolve a different version,
# then assert TOPOLOGY_VERSION overrides it.
TMPDIR_FAKE="$(mktemp -d)"
trap 'rm -rf "$TMPDIR_FAKE"' EXIT

mkdir -p "$TMPDIR_FAKE/scripts"
cp "$FETCH_SCRIPT" "$TMPDIR_FAKE/scripts/fetch-gatekeeper.sh"
