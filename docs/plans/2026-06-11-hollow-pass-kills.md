...
Branch: `feat/hollow-pass-kills` (worktree `topology-phase14`). The approval commit (task 3)
was originally planned as constitutionally human; the maintainer instead delegated it
in-session ("you have full control include git push or commit"), so per spec §4 it is
agent-executed with the honest trailer and becomes the **negative dogfood**: the
human-commit check must *fail* on it, proving it detects the delegated-approval practice.
...
| 3 | Approval commit: flip `Status: draft → approved` (one line; the revision marker is deliberately *not* on that line) | `docs/specs/…` | agent (maintainer-delegated, recorded) | design gate passes; honest agent trailer + `Approved-by:` note — task 7's **negative** dogfood (spec §4) |
