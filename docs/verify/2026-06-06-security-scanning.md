...
| `install.sh` registers PreToolUse matcher + links git pre-commit | static: `grep '"PreToolUse"'`, `'Bash\|Write\|Edit\|MultiEdit'`, `'pre-commit.sh'` all present; throwaway-copy run advertises PreToolUse and creates the symlink (live `.git` untouched) | pass |
