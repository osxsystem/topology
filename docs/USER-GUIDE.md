...
1. **Acquires the framework** via the **distribution payload** tarball — a curated, checksum-verified snapshot of just the operators (hooks, skills, instincts, scan rules, `scripts/fetch-gatekeeper.sh`, `AGENTS.md`, `VERSION`) — and unpacks it at `${TOPOLOGY_HOME:-$HOME/.topology}` (global) or `<path>/.topology` (local). Both scopes use the same download, checksum verification, and unpack machinery. Re-running upgrades an existing payload install in place; a `VERSION` file records the installed version so `gatekeeper doctor` can detect binary↔payload skew. If a legacy clone-based install is found, the installer rescues any in-tree state (learn ledger, memory handoffs) before prompting to replace. For global scope, rescued files land in a timestamped backup sibling (`${TOPOLOGY_HOME}-backup-<ts>/`); for local scope they go to `<path>/.claude/topology/`.
